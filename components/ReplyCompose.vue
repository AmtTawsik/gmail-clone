...
const { item, isReply } = defineProps(["item", "isReply"]);
const replyEditor = useState("replyEditor", () => null);
const content = ref("");
const recipient = ref(item.mail);
const isMinimize = ref(false);
const emit = defineEmits(['sendMail','isSendingMail','isShowToast']);


onMounted(() => {
    replyEditor.value = new Editor({
        content: content.value,
        onUpdate: ({ replyEditor }) => {
            content.value = replyEditor.getHTML()
        },
        editorProps: {
            attributes: {
                class: 'prose prose-sm sm:prose lg:prose-lg xl:prose-2xl min-h-[12rm] max-h-[12rm] mx-auto focus:outline-none',
            },
        },
        extensions: [
...
const setBold = () => {
    replyEditor.value.chain().focus().toggleBold().run();
};

const setStrikethrough = () => {
    replyEditor.value.chain().focus().toggleStrike().run();
};

const toggleItalic = () => {
    replyEditor.value.chain().focus().toggleItalic().run();
};

const toggleUnderline = () => {
    replyEditor.value.chain().focus().toggleUnderline().run();
};

const toggleBulletList = () => {
    replyEditor.value.chain().focus().toggleBulletList().run();
};

const toggleOrderedList = () => {
    replyEditor.value.chain().focus().toggleOrderedList().run();
};
...
<template>
    <div>
        ...
        <div class="drop-shadow-2xl sm:right-16 text-sm shadow-lg shadow-cyan-500/200 rounded-t-lg z-30 overflow-hidden"
            :class="{
            'sm:w-[17rem] w-full bottom-0 h-[2.5rem]': isMinimize,
            'w-full sm:w-[30rem] h-[20rem]': !isMinimize,
        }">
            <div
                ...
                <p class="text-[#172846] dark:text-green-real">Reply to {{ item.name }} ({{ item.email }})</p>
                <div class="flex items-center space-x-2">
                    ...
                    <button @click="cancelReply" class="cursor-pointer dark:text-green-real">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                            stroke="currentColor" class="w-4 h-4">
                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            'h-[305px] w-full': !isMinimize,
            hidden: isMinimize,
        }">
                <div
                    class="h-[8rem] overflow-hidden px-3 max-h-[200px] overflow-y-auto [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300">
                    <EditorContent :editor="replyEditor" v-model="content" data-hs-editor-field />
                </div>
                ...
                <div class="bg-white dark:bg-green-dark-light absolute bottom-12 bg-transparent w-full py-2">
                    ...
                    <div class="flex items-center px-2 justify-between">
                        <div class="flex items-center">
                            <button type="submit"
                                class="block bg-blue-600 dark:bg-green-real px-3 py-2 rounded-full sm:px-6 text-white mr-1">
                                <span>Send</span>
                            </button>
                        ...
                        <div>
                            <button type="reset" class="block" @click="cancelReply">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
